    ...
    def __init__(
        ...
        with BuildPart() as p_base_plate:
            ...
            with BuildSketch(Plane.XZ.offset(0)) as s:
                Trapezoid(
                    par.base_plate.trap_width,
                    par.base_plate.trap_height,
                    par.base_plate.trap_angle,
            ...
            sel = (
                faces().filter_by(Axis.Y).sort_by(Axis.Y)[1].edges().sort_by(Axis.Z)[0]
            )
            ...
            RigidJoint(
                label="j1",
                joint_location=Location(Plane(sel @ 0.5, (1, 0, 0), (0, -1, 0))),
            )

        super().__init__(
            part=p_base_plate.part, rotation=rotation, align=align, mode=mode
        )
